...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const BLESSING_TITLE_TEXT = "以女书寄女性";
    ...
    function renderQincheNushu() {
        const qin = nushuDict['秦'] || '秦';
        const che = nushuDict['彻'] || '彻';
    ...
    function renderBlessingTitle() {
        document.getElementById('blessingTitleNushu').innerHTML =
            buildNushuHtml(BLESSING_TITLE_TEXT);
    }
    ...
    window.askHomophone = function(index, oChar) {
        ...
        if (iChar && nushuDict[iChar[0]]) {
            ...
            document.getElementById(`action-${index}`).innerHTML =
                `<span style="color:#9d2933;font-size:12px;">已用「${iChar[0]}」女书</span>`;
    ...
    window.keepOriginal = function(index, char) {
        ...
        document.getElementById(`action-${index}`).innerHTML =
            `<span style="color:#425066;font-size:12px;">已保留</span>`;
    };
    ...
    document.getElementById('deconstructBtn').addEventListener('click', () => {
        ...
        const timer = setInterval(() => {
            ...
            if (frame >= 5) {
                clearInterval(timer);
                document.getElementById('goBlessingBtn').classList.remove('hidden');
            }
        }, 1000);
    });
    ...
    function startBackgroundFlow() {
        // 立即生成首批，错开出现时间
        for (let i = 0; i < 5; i++) {
            setTimeout(() => spawnRandomOther(), i * 700);
        }
        // 之后持续补充
    ...
    document.getElementById('sendBlessingBtn').addEventListener('click', async () => {
        ...
        const titleArea  = document.getElementById('blessingTitleArea');
        const sendBtn    = document.getElementById('sendBlessingBtn');
        [inputArea, titleArea, sendBtn].forEach(el => { el.style.opacity = '0'; });
        setTimeout(() => {
            [inputArea, titleArea, sendBtn].forEach(el => { el.style.display = 'none'; });
        }, 1000);

        // 背景河流显现
        document.getElementById('riverBg').style.opacity = '1';

        // ⑤ 用户寄语：恢复原版 flowMergeUp 动画
    ...
    function spawnFlowText(text, isUser) {
        ...
        // isUser 时 left/top 由 CSS keyframe flowMergeUp 控制（从 50% 中心出发）
        ...
        // 动画结束后自动移除（26s 对应 flowUp；flowMergeUp 是 22s）
        setTimeout(() => flowEl.remove(), isUser ? 23000 : 26000);
    }
});
